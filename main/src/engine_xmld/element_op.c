...
XMLDStatus engine_xmld_set_curr_att_value(FILE *fd, char *value) {
 char tokens[1];
 tokens[0]=fgetc(fd);
...
XMLDStatus engine_xmld_set_text_value(FILE *fd, char *value) {
...
XMLDStatus engine_xmld_locate_text(FILE *fd) {
 char *tok[]={"/>", ">"};
...
XMLDStatus engine_xmld_locate_att(FILE *fd, char *col_name) {
 int token;
 while (1) {
  token=dmstrchr(fd, " >", 2);
